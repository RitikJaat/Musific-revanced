...
import './App.css';
import './assets/playlist-styles.css';
  ...
  const handleViewPlaylist = async (playlistId, playlistName) => {
    ...
    try {
      ...
      if (response && response.success && response.songs) {
        console.log(`Loaded playlist with ${response.songs.length} songs. Displaying all songs.`);
        
        // Set the active playlist with all songs
        ...
        console.error("Failed to load playlist songs:", response);
      }
    } catch (error) {
            ...
            <>
              ...
              {searchActivePlaylist && searchActivePlaylistSongs.length > 0 ? (
                <div className="search-playlist-view mb-8 overflow-x-hidden">
                  ...
                  {/* Playlist songs in vertical list - display ALL songs without any limit */}
                  <div className="playlist-songs">
                    {/* Map over ALL songs in the playlist - no slicing or limiting */}
