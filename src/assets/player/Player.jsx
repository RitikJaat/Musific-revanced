...
const Player = () => {
  ...
  return (
    <div className="flex flex-col items-center group/he select-none">
      ...
      <div className="z-30 flex flex-col w-40 h-20 transition-all duration-300 bg-[#1b233d] shadow-md group-hover/he:h-40 group-hover/he:w-72 rounded-2xl shadow-zinc-400">
        <div className="flex flex-row w-full h-0 group-hover/he:h-20">
            ...
            {songImage ? (
              <div className="w-24 h-24 overflow-hidden rounded-full">
                <img 
                  src={songImage} 
                  alt="Album Cover" 
                  className="w-full h-full duration-500 border-4 rounded-full shadow-md border-zinc-400 border-spacing-5 object-cover"
                />
              </div>
            ) : (
          ...
          <div className="flex flex-col justify-center w-full pl-3 -ml-24 overflow-hidden group-hover/he:-ml-3 text-nowrap">
            <p className="text-xl font-bold">{currentSong?.name || 'No Song Selected'}</p>
            <p className="text-[rgba(170,222,243,0.721)]">{artistNames}</p>
          </div>
        </div>
        
        {/* Progress bar and time display - hidden in compact view */}
        <div className="hidden group-hover/he:flex flex-row items-center mx-3 mt-0">
          <span className="text-xs pl-1 text-[rgba(170,222,243,0.721)]">{formatTime(currentTime)}</span>
          <input 
            type="range" 
            min={0} 
            max={duration || 0}
            ...
            ref={progressBarRef}
            className="w-full flex-grow h-1 mx-2 my-auto bg-gray-300 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-zinc-400 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:shadow-md" 
          />
          <span className="text-xs pr-1 text-[rgba(170,222,243,0.721)]">{formatTime(duration)}</span>
        </div>

        {/* Simplified progress bar for compact view - no time display */}
        <div className="flex group-hover/he:hidden flex-row items-center justify-center mx-3 mt-1">
          <input 
            type="range" 
            min={0} 
            max={duration || 0}
        ...
        {/* Control buttons - now hidden in compact mode, visible only on hover */}
        <div className="flex-row items-center justify-center flex-grow mx-3 space-x-5 hidden group-hover/he:flex">
          <label htmlFor="playMode" className="flex items-center justify-center w-12 h-full cursor-pointer">
            <input 
              type="checkbox" 
              id="playMode" 
            ...
            />
            <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={isRepeat ? "#aadeef" : "#777"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="feather feather-repeat">
          ...
          <div className="flex items-center justify-center w-12 h-full cursor-pointer" onClick={skipToPrevious}>
          ...
          <div className="flex items-center justify-center w-12 h-full cursor-pointer" onClick={skipToNext}>
          ...
          <div className="flex items-center justify-center w-12 h-full cursor-pointer" onClick={toggleShuffle}>
            <svg xmlns="http://www.w3.org/2000/svg" width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={isShuffleMode ? "#aadeef" : "#777"} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="feather feather-shuffle">
        ...
        <div className="flex flex-row items-center justify-center flex-grow mx-3 group-hover/he:hidden">
          <div 
            className="flex items-center justify-center w-12 h-full cursor-pointer" 
            onClick={handlePlayPauseToggle}
          >
